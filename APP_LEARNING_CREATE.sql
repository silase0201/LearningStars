...
CREATE TABLE IF NOT EXISTS `star_option` (
    ...
    `primary_dimension_name` VARCHAR(40) NOT NULL,
    `secondary_dimension_name` VARCHAR(40) NOT NULL,
    `answer_meaning` VARCHAR(255) NOT NULL,
    `state` TINYINT(1) NOT NULL DEFAULT 1,
    `created_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updated_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),
    PRIMARY KEY (`id`),
  ...
  COMMENT='Question options and all T/E/G plus six-dimension scores imported from score.csv';
...
INSERT INTO `star_option`
    (`question_id`, `answer_key`, `option_code`, `option_text`,
     ...
     `primary_dimension_name`, `secondary_dimension_name`, `answer_meaning`, `state`)
VALUES
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q1'), 'Q1A', 'A', '看任務說明', 2,0,0, 2,0,0,0,0,1, '目標驅動', '觀察理解', '先確認任務內容與完成方向', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q1'), 'Q1B', 'B', '研究按鈕怎麼用', 0,2,0, 0,2,0,0,0,1, '探索思考', '觀察理解', '研究控制台按鈕的作用', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q1'), 'Q1C', 'C', '直接按按鈕試試', 0,0,2, 0,0,2,1,0,0, '持續嘗試', '問題解決', '透過操作按鈕直接開始嘗試', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q1'), 'Q1D', 'D', '想一個新玩法再試試', 0,1,1, 0,0,1,0,2,0, '創意表達', '持續嘗試', '自己想出新玩法並動手嘗試', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q2'), 'Q2A', 'A', '找找開盒提示', 2,0,0, 1,0,0,2,0,0, '問題解決', '目標驅動', '尋找能幫助打開盒子的提示', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q2'), 'Q2B', 'B', '仔細研究盒子機關', 0,2,0, 0,2,0,0,0,1, '探索思考', '觀察理解', '仔細研究盒子的構造與機關', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q2'), 'Q2C', 'C', '想不同方法試試', 0,0,2, 0,0,1,0,2,0, '創意表達', '持續嘗試', '想出不同做法並嘗試', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q2'), 'Q2D', 'D', '先觀察，再猜一個方法試試', 0,1,1, 0,2,0,0,0,1, '探索思考', '觀察理解', '先觀察線索，形成猜想後再試', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q3'), 'Q3A', 'A', '換一條航線', 0,0,2, 0,0,0,2,1,0, '問題解決', '創意表達', '改用另一條路線前進', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q3'), 'Q3B', 'B', '找出隕石規律', 0,2,0, 0,0,0,2,0,1, '問題解決', '觀察理解', '觀察並找出隕石移動規律', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q3'), 'Q3C', 'C', '確認方向再前進', 2,0,0, 2,0,0,1,0,0, '目標驅動', '問題解決', '確認目的地與方向後前進', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q3'), 'Q3D', 'D', '停一下再試', 1,0,1, 1,0,2,0,0,0, '持續嘗試', '目標驅動', '先暫停調整，之後再嘗試', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q4'), 'Q4A', 'A', '挑戰星球｜完成任務', 2,0,0, 2,0,0,1,0,0, '目標驅動', '問題解決', '喜歡朝明確任務前進', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q4'), 'Q4B', 'B', '發現星球｜找出祕密', 0,2,0, 0,2,0,0,0,1, '探索思考', '觀察理解', '喜歡探索並理解未知內容', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q4'), 'Q4C', 'C', '探險星球｜邊冒險邊想新玩法', 0,0,2, 0,0,2,0,1,0, '持續嘗試', '創意表達', '喜歡從實際體驗累積經驗，也會加入自己的玩法', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q4'), 'Q4D', 'D', '創造星球｜動手完成作品', 1,0,1, 1,0,0,0,2,0, '創意表達', '目標驅動', '喜歡動手創作並完成作品', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q5'), 'Q5A', 'A', '闖關任務書｜一關一關完成', 2,0,0, 2,0,0,1,0,0, '目標驅動', '問題解決', '偏好有明確關卡與完成目標的內容', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q5'), 'Q5B', 'B', '趣味發現書｜認識新奇事物', 0,2,0, 0,2,0,0,0,1, '探索思考', '觀察理解', '偏好認識與理解新奇事物', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q5'), 'Q5C', 'C', '實驗探索書｜動手找方法', 0,0,2, 0,2,0,0,1,0, '探索思考', '創意表達', '透過實驗探索並動手尋找方法', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q5'), 'Q5D', 'D', '創意故事書｜想出故事並完成', 1,1,0, 0,1,0,0,2,0, '創意表達', '探索思考', '加入自己的想法並完成故事', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q6'), 'Q6A', 'A', '任務導航員｜朝目標完成', 2,0,0, 2,0,0,1,0,0, '目標驅動', '問題解決', '朝清楚目標完成任務', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q6'), 'Q6B', 'B', '星際解謎家｜找出謎題答案', 0,2,0, 0,1,0,2,0,0, '問題解決', '探索思考', '找線索並解開謎題', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q6'), 'Q6C', 'C', '勇氣探險家｜看清狀況再試', 0,0,2, 0,0,2,0,0,1, '持續嘗試', '觀察理解', '看清狀況後繼續嘗試', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q6'), 'Q6D', 'D', '創意發明家｜用新方法完成', 1,1,0, 0,0,0,1,2,0, '創意表達', '問題解決', '使用新的方法完成任務', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q7'), 'Q7A', 'A', '先看到完成的樣子', 0,0,0, 2,0,0,0,0,1, '目標驅動', '觀察理解', '看到預期成果後較容易確認方向', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q7'), 'Q7B', 'B', '先找到可參考的線索', 0,0,0, 0,1,0,0,0,2, '觀察理解', '探索思考', '找到可觀察的線索後較容易理解任務', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q7'), 'Q7C', 'C', '可以直接動手試試', 0,0,0, 0,0,2,1,0,0, '持續嘗試', '問題解決', '透過直接操作開始嘗試', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q7'), 'Q7D', 'D', '可以用自己的方法', 0,0,0, 0,0,0,1,2,0, '創意表達', '問題解決', '能使用自己的做法時較容易開始', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q8'), 'Q8A', 'A', '再問問原因', 0,0,0, 0,2,0,1,0,0, '探索思考', '問題解決', '主動追問以理解原因', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q8'), 'Q8B', 'B', '請人再示範一次', 0,0,0, 0,1,0,0,0,2, '觀察理解', '探索思考', '透過再次觀察示範建立理解', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q8'), 'Q8C', 'C', '邊操作邊觀察', 0,0,0, 0,0,2,0,0,1, '持續嘗試', '觀察理解', '透過再次操作與觀察理解', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q8'), 'Q8D', 'D', '回頭查看步驟', 0,0,0, 0,0,0,1,0,2, '觀察理解', '問題解決', '回頭確認原有步驟，再照著處理', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q9'), 'Q9A', 'A', '找出錯在哪裡', 0,0,0, 0,0,0,2,0,1, '問題解決', '觀察理解', '仔細觀察並定位錯誤', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q9'), 'Q9B', 'B', '換個方法再試', 0,0,0, 0,0,1,0,2,0, '創意表達', '持續嘗試', '改用不同方法繼續嘗試', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q9'), 'Q9C', 'C', '繼續試到完成', 0,0,0, 2,0,1,0,0,0, '目標驅動', '持續嘗試', '以完成拼圖為方向持續嘗試', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q9'), 'Q9D', 'D', '看完別人再試', 0,0,0, 0,0,1,0,0,2, '觀察理解', '持續嘗試', '先觀察他人做法，再回來嘗試', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q10'), 'Q10A', 'A', '知道離完成還有多遠', 0,0,0, 2,0,1,0,0,0, '目標驅動', '持續嘗試', '能看見自己與完成目標之間的距離', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q10'), 'Q10B', 'B', '過程中一直有新發現', 0,0,0, 0,2,0,0,0,1, '探索思考', '觀察理解', '在過程中持續看見新的內容', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q10'), 'Q10C', 'C', '可以嘗試不同方法', 0,0,0, 0,0,2,0,1,0, '持續嘗試', '創意表達', '可以透過不同方法持續嘗試', 1),
    ((SELECT `id` FROM `star_question` WHERE `quiz_id`=@star_quiz_id AND `question_code`='Q10'), 'Q10D', 'D', '可以加入自己的想法', 0,0,0, 1,0,0,0,2,0, '創意表達', '目標驅動', '可以將自己的想法放進任務中', 1)
ON DUPLICATE KEY UPDATE
    ...
    `primary_dimension_name` = VALUES(`primary_dimension_name`),
    `secondary_dimension_name` = VALUES(`secondary_dimension_name`),
